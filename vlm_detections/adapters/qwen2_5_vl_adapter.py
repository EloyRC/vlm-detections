...
class Qwen25VLAdapter(PromptBasedVLM, GenerativeVLMBase):
    """
    Adapter for Qwen2.5-VL models via Hugging Face Transformers.
    Generates JSON with bounding boxes using an instruction prompt and parses detections.
    """

    def __init__(self, model_id: str = "Qwen/Qwen2.5-VL-2B-Instruct"):
    ...
    def name(self) -> str:
        return f"Qwen2.5-VL ({self.model_id})"
    ...
    # Prompts are provided by the caller; no in-code defaults here.

    def _generate(self, inputs) -> List[str]:  # type: ignore
        if self.model is None or self.processor is None:
            raise RuntimeError("Qwen2.5-VL adapter not loaded. Call load() first.")
    ...
    def _generate_sequences(
        ...
        sanitized_prompt = (prompt_text or "").strip()
        messages = build_qwen_messages((system_prompt or "").strip(), pil_images, sanitized_prompt)
    ...
    def infer_from_batch(
        ...
        """Single call with multiple images passed to Qwen chat template."""
        if self.model is None or self.processor is None:
            raise RuntimeError("Qwen2.5-VL adapter not loaded. Call load() first.")
    ...
    # --- Chat support ---
    def chat_infer(self, conversation, threshold: float = 0.0) -> str:  # threshold unused in pure text mode
        if self.model is None or self.processor is None:
            raise RuntimeError("Qwen2.5-VL adapter not loaded. Call load() first.")
